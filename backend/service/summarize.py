...
凡是数学/物理公式、变量、上下标,一律用 LaTeX 表示:行内公式用 $...$ 包裹,独立成行的公式用 $$...$$ 包裹。例如双曲正弦写成 $\\operatorname{sh} x=\\frac{e^x-e^{-x}}{2}$、去心邻域写成 $0<|x-a|<\\delta$,绝不要写成 sh x=(e^x-e^{-x})/2、0<|x-a|<δ 这种纯文本上标/花括号形式。注意输出的是 JSON,LaTeX 里的反斜杠必须转义成两个反斜杠(如 \\\\frac、\\\\delta)以保证 JSON 合法。
    ...
    def summarize(self, lines, title=None, board="", lang="zh-Hans", syllabus=None, materials=None):
        ...
        if mats:
            ...
            mat_sec = (
                ...
                "③ 资料里有、但这节课**没讲**、老师也没提的内容不要写进来(摘要只反映这节课);"

                "④ 凡是借助资料补充或校正过的要点,在该条开头加上「【资料】」标记(可带文件名,如「【资料·讲义】」);"
                "**这个标记优先于「【教材】」**——同一条既参考了资料又属于教材内容时,只标「【资料】」。"
                "如果某份资料和这节课其实无关,忽略它、不要硬凑。"
                "\n\n**【必做】老师跳过的地方要补全**:老师说「你们自己看书」「课后自学」「我就不展开了」"
                "「自己去看」「这部分不讲了」时,他是把这一段**留给学生自学**,而学生正是靠这份笔记复习的。"
                "遇到这种地方,**必须**从上面的资料里找到对应章节,把老师跳过的内容补成几条 key_points"
                "(定义、公式、性质、例子各成一条,能补多少补多少,通常 3~8 条),每条开头标「【自学补充】」"
                "(可带出处,如「【自学补充·§1.1】」)。这是本条规则的**唯一例外**——其余情况仍只写课上讲过的。"
                "同时在 summary 里点明「这部分老师让自学,已按教材补出要点」。资料里确实找不到才不补。")
        ...
        if mats:
            ...
            user += ("\n\n【标记优先级(最终规则)】本次有用户提供的课程资料。凡是参考了上面这些资料的要点,"
                     ...
                     "老师让自己看书、你据资料补出来的条目标「【自学补充】」,它同样优先于「【教材】」。")
        payload = json.dumps({
            "model": self.model,
