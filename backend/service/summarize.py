...
class DeepSeek:
    ...
    def segment(self, fragments, topic="", timeout_s=12):
        ...
        sys_prompt = (
            "你在做中文语音转写的智能分句。下面是按时间先后排列的若干 ASR 碎片,每个碎片是说话人"
            "一次停顿切出来的——一句话可能被切成好几段,一段里也可能含好几句。请**只按语意**把开头"
            "连续的碎片合并成一个个**完整的句子**(每句末尾加合适的中文标点 。/?/!)。碎片的先后顺序"
            "和字词都不要改、不要增删内容,只做合并与加标点。若结尾几个碎片明显还没说完(半句话),"
            "把它们留下、先不要成句。"
            '严格输出 JSON:{"commit":[{"n":<吃掉开头几个碎片的个数>,"text":"<合成的完整句,带标点>"}...],'
            '"tail_n":<结尾没说完、保留的碎片个数>}。要求所有 commit 的 n 之和 加 tail_n 恰好等于碎片总数;'
            "若整段都还没说完,commit 为空数组、tail_n 等于碎片总数。只输出 JSON,不要解释。")
        topic = (topic or "").strip()
        if topic:
