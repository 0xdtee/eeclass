...
class AliyunGummyBackend(Backend):
    """Alibaba Cloud · Gummy (gummy-realtime-v1): multilingual speech recognition.

    Recognizes cfg['gummy']['source'] (zh/en/fr/de/it/ja/ko/es/ru). Recognition only -- Gummy's built-in
    translation reaches just a few targets (zh/en/ja/ko), so translation for this backend goes through DeepSeek
    like every other model, which covers all target languages uniformly.
    ...
    def transcribe(self, audio, with_prompt=True):
        ...
        try:
            ...
            return ""
        return ("".join(got["orig"]) or got["orig_last"]).strip()
