    ...
    <div className="min-h-screen bg-background-100">
      <nav className="sticky top-0 z-30 bg-background-50/95 backdrop-blur-sm border-b border-background-200">
        <div className="flex items-center justify-between h-14 px-6 max-w-6xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-semibold text-foreground-900">{t('参考资料 · 官方教学大纲')}</h1>
              <p className="text-xs text-foreground-400">{t('选择学校 → 查看该校教务处发布的课程教学大纲 PDF')}</p>
            </div>
          </div>
          {item && (
            <a href={isPage ? (item.source_page || pdfSrc) : pdfSrc} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-4 py-2 bg-primary-500 text-background-50 rounded-full text-xs font-semibold hover:bg-primary-600 cursor-pointer whitespace-nowrap">
              <i className="ri-external-link-line"></i>{isPage ? t('打开原网页') : t('新标签打开')}
            </a>
          )}
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col md:flex-row gap-5 items-start">
        ...
        <div className="w-full md:w-64 flex-shrink-0 space-y-4">
          {grouped.map((c) => (
            <div key={c.name}>
              <p className="text-xs font-semibold text-foreground-400 mb-1.5 flex items-center gap-1.5"><i className={c.icon}></i>{t(c.name)}</p>
              <div className="space-y-1">
                {c.list.map((it) => (
                  ...
                  >
                    ...
                    {it.kind === 'page' && (
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium whitespace-nowrap ${course === it.course ? 'bg-background-50/25 text-background-50' : 'bg-sky-100 text-sky-700'}`}>{t('网页')}</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="bg-background-50 border border-background-200 rounded-xl px-5 py-3 flex items-center gap-2 flex-wrap">
                <span className="text-xs px-2 py-0.5 bg-green-100 text-green-700 rounded-full font-semibold whitespace-nowrap">{school?.name}{t('官方')}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium whitespace-nowrap ${isPage ? 'bg-sky-100 text-sky-700' : 'bg-background-100 text-foreground-500'}`}>{isPage ? t('网页版') : 'PDF'}</span>
                <h2 className="text-sm font-bold text-foreground-900 truncate">{item.title}</h2>
                {item.source_page && (
                  <a href={item.source_page} target="_blank" rel="noreferrer" className="ml-auto text-xs text-primary-500 hover:underline whitespace-nowrap"><i className="ri-links-line mr-1"></i>{t('来源页')}</a>
                )}
              </div>
              ...
              <div className="relative">
                {pdfLoading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-background-50/80 rounded-xl z-10">
                    ...
                    <p className="text-sm text-foreground-400">{isPage ? t('正在加载官方网页大纲…') : t('正在加载官方 PDF…')}</p>
                  </div>
                )}
                <iframe
                  title={t('官方教学大纲')}
              ...
              <p className="text-[11px] text-foreground-300 text-center">{t('若长时间空白,可能是该校资料需校内网访问,点右上')}{isPage ? t('「打开原网页」') : t('「新标签打开」')}{t('或「来源页」试试。')}</p>
            </div>
          ) : (
            <p className="text-sm text-foreground-400 py-24 text-center">{t('这所学校暂时还没有收录课程大纲。')}</p>
          )}
        </div>
      </div>
    </div>
