...
export default function SyncShuModal({ isOpen, onClose, onSync, onConfirmEvents }: SyncShuModalProps) {
  ...
  const go = async () => {
    if (!u.trim() || !p) { setErr(t('请输入学/工号和密码')); return; }
    setBusy(true); setErr(''); setMsg('');
    try {
      const r = await onSync(u.trim(), p);
      if (r.error) { setErr(r.error); }
      else { setEvents(r.events || []); setMsg(r.note || t('完成')); }
    } catch (e) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={close} title={t('上大选课 → 同步日历')} width="max-w-md">
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-background-200 p-3.5">
          ...
          >
            <i className="ri-external-link-line"></i>{t('打开上大选课官网')}
          </a>
          <p className="text-[11px] text-foreground-400 mt-2 leading-relaxed">
            {t('在官网正常登录、选课、点确定。选好后回到这里做第二步——把已选课程同步进日历。')}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-background-200 p-3.5 space-y-3">
          ...
          <p className="text-[11px] text-foreground-400 leading-relaxed">
            {t('用你的')}<b>{t('上大统一身份认证')}</b>{t('学工号+密码登录教务系统,抓取你已选的课表加进日历。密码')}<b>{t('只用于这次登录、不保存')}</b>。
          </p>

          <div>
            <label className="block text-xs font-medium text-foreground-600 mb-1.5">{t('学 / 工号')}</label>
            <input
              value={u}
          ...
          {msg && (
            <div className="text-xs text-foreground-600 bg-background-100 rounded-lg p-3">
              ...
              {events.length > 0 && <span className="ml-1">{t('识别到 {n} 节课。', { n: events.length })}</span>}
            </div>
          )}
          ...
          {events.length > 0 ? (
            ...
            >
              {busy ? t('登录并同步中…(约 20 秒)') : t('同步已选课程到日历')}
            </button>
          )}
        </div>
      </div>
