...
export default function TagCoursesModal({ isOpen, onClose, groups, onSelect, onManage }: TagCoursesModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('按标签整理课程')} width="max-w-lg">
      <div className="flex flex-col" style={{ maxHeight: '70vh' }}>
        ...
        <div className="flex items-center justify-between px-1 pb-4 border-b border-background-100 mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs text-foreground-400">{t('用到的标签(按标签整理课程与录音)')}</p>
              <p className="text-2xl font-bold text-foreground-900">{t('{n} 个', { n: groups.length })}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {onManage && (
              <button
                ...
                title={t('管理标签')}
              >
                <i className="ri-settings-3-line text-xs"></i>{t('管理标签')}
              </button>
            )}
            ...
            <div className="py-10 text-center">
              {groups.length === 0 ? (
                <>
                  ...
                  <p className="text-sm font-medium text-foreground-500 mb-1">{t('还没有带标签的课程')}</p>
                  <p className="text-xs text-foreground-300 mb-4">{t('用图片导入课表时会按课名自动给课打标签;也可到某节课的「录音集合」给录音打标签,之后就能在这里按标签整理。')}</p>
                  {onManage && (
                    <button
                      onClick={onManage}
                    ...
                    >
                      <i className="ri-settings-3-line"></i>{t('去管理标签')}
                    </button>
                  )}
                </>
              ) : (
                <p className="text-sm text-foreground-400">{t('没找到匹配的标签')}</p>
              )}
            </div>
          ) : (
            filtered.map((g, idx) => (
                ...
                >
                  ...
                  {g.courses.length > 0 && (
                    <span className="px-2 py-0.5 bg-secondary-100 text-secondary-700 rounded-full text-[11px] font-medium whitespace-nowrap flex-shrink-0">
                      <i className="ri-book-2-line mr-0.5"></i>{t('{n} 门课', { n: g.courses.length })}
                    </span>
                  )}
                  {g.count > 0 && (
                    <span className="px-2 py-0.5 bg-primary-100 text-primary-700 rounded-full text-[11px] font-medium whitespace-nowrap flex-shrink-0">
                      <i className="ri-mic-line mr-0.5"></i>{t('{n} 段录音', { n: g.count })}
                    </span>
                  )}
