...
> **把一节课的现场讲授,实时变成一份干净、结构化、可搜索、可复习的笔记 —— 而且音频全程留在你自己的机器上。**

eeclass 是一套**自托管**的课堂转写与 AI 笔记系统。老师在台上讲,它在本机 CPU 上实时出字幕、分辨说话人、标记重点;课后再用 AI 生成摘要、考点、模拟卷,帮你复习。语音识别、说话人区分、声纹全部**在本地跑,音频不出设备**;只有可选的纯文本调用(纠错/摘要)才会发给外部大模型。
...
- **注重隐私的个人与机构** —— 不愿把课堂录音上传到第三方云,要求全程本地、自己掌控数据。
...
- **云转写的隐私顾虑** —— 音频、识别、声纹全在本机 CPU 完成,不上传。
...
- 🔒 **全本地、音频不出设备** —— ASR、说话人、声纹都在 CPU 本地跑(sherpa-onnx SenseVoice,RTF≈0.05),只有可选的文本纠错/摘要走外部 LLM。
...
- 🌏 **多语种 + 自动翻译** —— 中/英/日/韩/粤识别;英文句自动在下方挂中文字幕。
- 🏠 **自托管、开源(MIT)** —— 一台普通机器 + 局域网即可跑,数据全在自己手里。
...
- **AI 辅助(接 DeepSeek,可选)** —— 同音纠错、智能分句、英文→中文字幕、每节课摘要、课程大总结、考点预测、模拟试卷、闪卡/自测、对整节课"追问"问答。
- **拍板书** —— 一键拍下黑板/PPT,按时间轴对齐。
- **账号与隐私** —— 真实登录(pbkdf2)、严格的按账号数据隔离、声纹库仅管理员可管理、只读分享链接。
...
- **音频、识别、声纹 —— 全本地、CPU 完成,语音绝不离开本机。**
- 只有可选的**文本**(转写片段,用于纠错/摘要)会发给 DeepSeek。
