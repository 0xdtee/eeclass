...
- 🤝 **会议翻译(中 ⇄ 英)** —— 独立的会议互译页:说英文自动出中文、说中文自动出英文,开会边说边看双语。用云端 Gummy 模型自动判别语种并一次识别 + 互译。
...
- **会议翻译** —— 独立的中英双向互译页(`/meeting`):自动判别说的是中文还是英文,翻成另一种语言,双语气泡实时滚动;用云端 Gummy 模型识别 + 互译,适合线下会议 / 洽谈。可全屏投影到大屏;点「结束」后自动生成**会议纪要**(概述 / 讨论要点 / 决定事项 / 待办含负责人,DeepSeek 生成,可复制)。历史记录按账号跨设备同步。
